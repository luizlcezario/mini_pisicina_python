...
class Elem:
    ...
    def __init__(self, tag='div', attr={}, content=None, tag_type='double'):
        ...
        if type(attr) is not dict:
            raise Elem.ValidationError("Error on validation of dict in " + tag)
        self.attr = attr
        if(type(content) == str):
    ...
    def __make_content(self):
        ...
        for elem in self.content:
            ...
            if not result.endswith('\n'):
                result += '\n'
                for i in range(0, elem.indent - 1 ):
                    result += '  ' 
        return result

    def add_content(self, content):
        if not Elem.check_type(content):
            raise Elem.ValidationError("Error tryng add " + content + " to " + self.tag)
